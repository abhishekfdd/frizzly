...
class Frizzly_Admin_Image_Submodule extends Frizzly_Admin_Button_Settings_Submodule_Base {
	...
	function get_page_settings( $db_value ) {
		...
		$settings['button_margin_top'] = array_merge(
			$margin_setting,
			array(
		...
		$settings['button_margin_left'] = array_merge(
			$margin_setting,
			array(
		...
		$settings['button_margin_right'] = array_merge(
			$margin_setting,
			array(
		...
		$settings['button_margin_bottom'] = array_merge(
			$margin_setting,
			array(
		...
		$settings['desktop_min_height'] = array_merge(
			array(
		...
		$settings['desktop_min_width'] = array_merge(
			array(
	...
	function get_page_i18n() {
		...
		$new_i18n    = array(
			...
			/* translators: 1: button size, 2: button shape, 3: when buttons are shown */
			'button_style_template'      => __( 'Share buttons should be %1$size% and %2$shape%. They should be displayed %3$show%.', 'frizzly' ),
			'min_resolution_title'       => __( 'Minimum image resolution', 'frizzly' ),
			/* translators: 1: minimum image height, 2: minimum image width */
			...
			/* translators: 1: with/without discriminator, 2: list of CSS classes */
			'image_classes_template'     => __( 'Select only images %1$discriminator% the following CSS classes: %2$classes%.', 'frizzly' ),
