<?php
/**
 * Frizzly Client Content Submodule.
 *
 * @package Frizzly
 */
...
class Frizzly_Client_Content_Submodule extends Frizzly_Client_Submodule {
	...
	private function add_to_content() {
		...
		$before  = in_array( $options['where'], array( 'before', 'before_after' ), true );
		$after   = in_array( $options['where'], array( 'after', 'before_after' ), true );
	 ...
	 */
	public function filter_add_pinterest_data( $content ) {
		...
		$options          = $this->get_submodule_options();
		$add_pinterest    = in_array( 'pinterest', $options['networks'], true );
		...
		foreach ( $images as $img ) {
			...
			foreach ( $attributes as $att ) {
				if ( $add_pinterest && in_array( $att[1], $atts_to_save, true ) ) {
			...
			if ( $add_pinterest ) {
				...
				if ( strlen( $link ) > 0 ) {
					$new_img .= sprintf( ' data-frizzly-content-share-pinterest="%s"', esc_attr( $link ) );
				}
			}

			$new_img .= sprintf( 'data-frizzly-content-post-id="%s">', $post_id );
	...
	/**
	 * Filter add share icons.
	 *
	 * @param mixed $content string.
	 * @return string
	 */
	public function filter_add_share_icons( $content ) {
		if ( ! $this->add_to_content() ) {
			return $content;
		}
		$options   = $this->get_submodule_options();
